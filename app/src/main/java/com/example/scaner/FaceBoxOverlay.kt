...
open class FaceBoxOverlay (context: Context?, attrs: AttributeSet?): View(context, attrs){
    ...
    abstract class FaceBox(private val overlay: FaceBoxOverlay) {
        ...
        fun getBoxRect(imageRectWidth: Float, imageRectHeight: Float, faceBoundBox: Rect): RectF {
            ...
            val mappedBox = RectF().apply {
                left = faceBoundBox.right * scale + offsetX
                top = faceBoundBox.top * scale + offsetY
                right = faceBoundBox.left * scale + offsetX
                bottom = faceBoundBox.bottom * scale + offsetY
            }
